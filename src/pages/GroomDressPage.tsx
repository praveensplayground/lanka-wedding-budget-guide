...
const GroomDressPage = () => {
  ...
  const handleNext = () => {
    ...
    const selectedOption = groomDressOptions.find(opt => opt.tier === selectedTier);
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-2 mb-4">
            <ShirtIcon className="h-6 w-6 text-rose-500" />
            <h1 className="text-3xl font-serif text-rose-800">Groom Dress</h1>
          </div>
          <p className="text-rose-600">Select the perfect attire for the groom</p>
        </div>
        <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto mb-8">
          {groomDressOptions.map((option) => (
            <Card 
              key={option.tier}
        ...
        <div className="flex justify-between items-center max-w-6xl mx-auto">
          <Button
            onClick={() => navigate('/bridal-dress')}
            variant="outline"
            className="border-rose-300 text-rose-700 hover:bg-rose-50"
          >
            <ArrowLeft className="h-4 w-4 mr-2" />
            Back to Bridal Dress
          </Button>
          <Button
            onClick={handleNext}
